...
public class PluggableSchemaResolver implements EntityResolver {
	...
	/**
	 *
	 * 案例：<beans xmlns="http://www.springframework.org/schema/beans"
	 *
	 * systemId则为: http://www.springframework.org/schema/beans
	 */
	@Override
	@Nullable
	public InputSource resolveEntity(@Nullable String publicId, @Nullable String systemId) throws IOException {
		...
		if (systemId != null) {
			// 从schemaMappings中拿到systemId对应文件的本地路径地址
			String resourceLocation = getSchemaMappings().get(systemId);

			// 如果本地没有此文件，并且还是走的https协议，那么从schemaMappings再通过http的方式再找一次
			...
			if (resourceLocation != null) {
				// 则使用ClassPathResource加载本地资源为resource对象
		...
		if (schemaMappings == null) {
			...
			synchronized (this) {
				...
				if (schemaMappings == null) {
					...
					try {
						// 加载所有的"META-INF/spring.schemas"配置
						// META-INF/spring.schemas下存储的就是以systemId为key，以xsd文件在jar包中的路径作为value的kv键值对
						Properties mappings =
