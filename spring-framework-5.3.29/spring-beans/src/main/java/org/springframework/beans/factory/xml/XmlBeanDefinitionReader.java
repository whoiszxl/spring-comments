...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		// EntityResolver实体解析器，作用是从jar包中获取xml文件对应的xsd，dtd声明文件的
		if (this.entityResolver == null) {
			// Determine default EntityResolver to use.
			// 获取resourceLoader
			ResourceLoader resourceLoader = getResourceLoader();
			if (resourceLoader != null) {
				// 如果ResourceLoader不为空,则使用ResourceEntityResolver,通过ResourceLoader解析实体
				this.entityResolver = new ResourceEntityResolver(resourceLoader);
			}
			else {
				// 否则使用DelegatingEntityResolver,通过BeanClassLoader解析实体
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			...
			// 传入输入流和resource，开始加载BeanDefinitions
			...
			// 最后移除threadLocal中的resource资源
			currentResources.remove(encodedResource);
			// 如果SET中resource空了，则threadLocal一并移除
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// 通过DefaultDocumentLoader的loadDocument方法做XML加载
	...
	protected int getValidationModeForResource(Resource resource) {
		// 获取配置的全局验证模式validationMode
		int validationModeToUse = getValidationMode();

		// 如果不是自动检测VALIDATION_AUTO,则直接返回配置的验证模式
		if (validationModeToUse != VALIDATION_AUTO) {
			return validationModeToUse;
		}

		// 如果是自动检测,则调用detectValidationMode方法去检测给定resource的验证模式
		int detectedMode = detectValidationMode(resource);
		// 如果检测到的不是自动检测,则返回检测到的验证模式
		...
		// 如果检测结果也是自动检测,则无法确定,默认假定为XSD验证(VALIDATION_XSD)
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			// 通过Resource的getInputStream()方法获取输入流。
		...
		try {
			// 使用validationModeDetector(实现自ValidationModeDetector接口)的detectValidationMode方法基于输入流内容检测验证模式
			// 实际上检测的是输入流是否包含DOCTYPE定义，包含则返回DTD验证，否则返回XSD验证
