...
public class ClassPathResource extends AbstractFileResolvingResource {
	...
	public ClassPathResource(String path, @Nullable ClassLoader classLoader) {
		//断言校验传入的path地址不能为null
		Assert.notNull(path, "Path must not be null");

		//对传入的文件地址进行规范化处理，消除不必要的路径元素（如"."和".."），并将Windows路径分隔符替换为正斜杠
		...
		// 确定要使用的类加载器，如果传入的类加载器为null，则使用线程上下文类加载器
	...
	public InputStream getInputStream() throws IOException {
		InputStream is;
		// 如果有指定关联的类，则尝试使用关联的类的 getResourceAsStream 方法获取输入流
		if (this.clazz != null) {
			is = this.clazz.getResourceAsStream(this.path);
		}
		// 如果没有关联的类，但有指定类加载器，则尝试使用类加载器的 getResourceAsStream 方法获取输入流
		else if (this.classLoader != null) {
			is = this.classLoader.getResourceAsStream(this.path);
		}
		// 如果都没有指定，则尝试使用系统类加载器的 getResourceAsStream 方法获取输入流
		else {
			is = ClassLoader.getSystemResourceAsStream(this.path);
		}

		// 如果获取的输入流为null，表示资源不存在，抛出FileNotFoundException
		if (is == null) {
